...
3. **Environment variable expansion**: Variables like `${PRIVATE_REPO_TOKEN}` are automatically expanded from environment variables. If a variable is unset, UniDep fails before invoking installers and names the missing variable.
...
```bash
usage: unidep [-h]
              {merge,install,install-all,conda-lock,pixi,pip-compile,pip,conda,doctor,version,docs} ...
...
positional arguments:
  {merge,install,install-all,conda-lock,pixi,pip-compile,pip,conda,doctor,version,docs}
    ...
    docs                Print the full README.md documentation.
